...
@Configuration
@EnableMongoRepositories(basePackages = "cn.datawin.dao.ai",
        mongoTemplateRef = "aiMongoHelper")
public class AiMongoConfig {

    private Logger logger = LoggerFactory.getLogger(getClass());
    @Bean(name = "aiMongoSettingsProperties")
    @ConfigurationProperties(
            prefix = "spring.data.mongodb.ai")
    ...
    // 覆盖默认的MongoDbFactory
    @Primary
    @Bean(name = "aiMongoDbFactory")
    MongoDbFactory mongoDbFactory(@Qualifier("aiMongoSettingsProperties")MongoSettingsProperties mongoSettingsProperties) {
        //客户端配置（连接数、副本集群验证）
    ...
    @Primary
    @Bean(name="aiMongoHelper")
    public MongoHelper mongoHelper(@Qualifier("aiMongoDbFactory") MongoDbFactory mongoDbFactory) throws Exception {
